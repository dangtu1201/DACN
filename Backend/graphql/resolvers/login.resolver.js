...
export const loginResolvers = {
    Query: {
      ...
      getAllShopLogins: async () => {
        return await Login.find({});
      },            
      getShopLogin: async (_, { ID }) => {
        return await Login.findById(ID);
      },
    },
    Mutation: {
      Login: async (_, { input }, { res }) => {
        ...
        console.log(`User ${rs.userID} logged in`);
        return rs;
      },
      ...
      LoginShop: async (_, { input }, { res }) => {
        ...
        let user = await Shop.findOne({phone: input.phone});
        //console.log("phone: ", user)
        if (!user) user = await Shop.findOne({email: input.phone});
        //console.log("email: ", user)
      ...
      LogoutShop: async (_, args, context) => {
        // const randomId = Math.random().toString().split('.')[1];
        // const newUser = { ...args, _id: randomId }
